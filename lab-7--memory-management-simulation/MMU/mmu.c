...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
  ...
  for (int i = 0; curr != NULL; i++) { 
    //if so, remove it and go to #3
    if ((curr->blk->end - curr->blk->start) >= blocksize) {
      count = 1;
      blk = list_remove_at_index(freelist,i);
      break;
    }
    curr = curr->next;
  }
  
  //if not print ""Error: Memory Allocation <blocksize> blocks\n""
  if(count == 0){
    ...
    //set the fragment->end = original blk.end before you changed it in #4
    fragment->end = end;
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
  ...
  bool isIn = false;
  for(int i = 0; curr != NULL; i++){
    if(curr->blk->pid == pid){ //if so, remove it and go to #3
      isIn = true;
      blk = list_remove_at_index(alloclist, i);
      break;
    }
    else{
      curr = curr->next;
  ...
  //if not print "Error: Can't locate Memory Used by PID: <pid>"
  if(isIn == false ){
